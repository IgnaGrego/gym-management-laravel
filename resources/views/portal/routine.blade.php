...
@section('title', 'Routine - El Area Gym')

@section('content')
    <section>
        <h1 class="text-3xl font-bold tracking-tight text-stone-900">Routine</h1>

        @include('partials.portal-nav')

        @if ($routine === null)
            <p class="mt-6 text-stone-600">You have no assigned routine yet.</p>
        @else
            <h2 class="mt-6 text-xl font-semibold text-stone-900">{{ $routine->name }}</h2>

            @foreach ($routine->days as $day)
                <h3 class="mt-4 text-lg font-semibold text-stone-800">Day {{ $day->day_number }}</h3>

                <ul class="mt-2 space-y-2">
                    @foreach ($day->exercises as $row)
                        <li class="rounded-lg border border-stone-200 bg-white p-4">
                            <p class="font-semibold text-stone-900">{{ $row->exercise?->name ?? '—' }} · Set {{ $row->set_number }}</p>
