...
@foreach ([['full_name','Nombre completo','text'],['dni','DNI','text'],['email','Email','email'],['phone','Teléfono','text'],['emergency_contact','Contacto de emergencia','text']] as [$name,$label,$type])
...
@endforeach
<label for="password">Contraseña</label><input id="password" type="password" name="password" required autocomplete="new-password">@error('password')<p class="error">{{ $message }}</p>@enderror
<label for="password_confirmation">Confirmar contraseña</label><input id="password_confirmation" type="password" name="password_confirmation" required autocomplete="new-password">
<label for="injuries_notes">Notas de lesiones</label><textarea id="injuries_notes" name="injuries_notes">{{ old('injuries_notes') }}</textarea>
<label for="medical_conditions_notes">Notas de condiciones médicas</label><textarea id="medical_conditions_notes" name="medical_conditions_notes">{{ old('medical_conditions_notes') }}</textarea>
<button type="submit">Registrarse</button><p><a href="{{ route('login') }}">¿Ya tienes una cuenta? Inicia sesión</a></p></form></body></html>
